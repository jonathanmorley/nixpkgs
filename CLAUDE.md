...
- **Personal** — `modules/personal/darwin.nix`, only imported when the `personal` profile is active (see `lib/mkDarwinSystem.nix`). Personal machines (`medusa`, `smoke`) enable it; the CI host (`gha-aarch64-darwin`, profiles = `[]`) does not.
